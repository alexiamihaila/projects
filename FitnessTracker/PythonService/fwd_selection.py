...
class ClassificationAlgorithms:
    ...
    def neural_network(
        ...
        model = tf.keras.Sequential([
            tf.keras.layers.Dense(num_hidden_units, activation='relu', input_shape=(train_X.shape[1],)),
        ...
        pred_train_prob_y = model.predict(train_X)
        pred_test_prob_y = model.predict(test_X)
        pred_train_y = tf.argmax(pred_train_prob_y, axis=1)
        pred_test_y = tf.argmax(pred_test_prob_y, axis=1)
